...
function LobbyContent() {
  ...
  async function handleRankedSearch() {
    ...
    if (error) { toast.error('Could not join matchmaking'); setIsSearching(false); return }
      ...
      .on('postgres_changes', {
        ...
        if (payload.new.status === 'matched') {
          supabase.from('game_rooms').select('id')
            ...
            .eq('status', 'drafting').order('created_at', { ascending: false }).limit(1).single()
  ...
  async function cancelSearch() {
    ...
    const supabase = createClient()
    await supabase.from('matchmaking_queue').update({ status: 'cancelled' })
  ...
  async function joinCasualRoom() {
    ...
    if (findErr || !room) { toast.error('Room not found or already started'); return }
    const { error: joinErr } = await supabase.from('game_rooms')
      .update({ player_b_id: user.id, status: 'drafting', started_at: new Date().toISOString() }).eq('id', room.id)
    if (joinErr) { toast.error('Failed to join room'); return }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      {/* Casual room modal */}
      <Modal open={casualOpen} onClose={() => setCasualOpen(false)} title="Room Created">
